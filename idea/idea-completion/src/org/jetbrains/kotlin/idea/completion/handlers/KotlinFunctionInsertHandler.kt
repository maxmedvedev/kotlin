    ...
    class Normal(
        ...
        private fun addArguments(context: InsertionContext, offsetElement: PsiElement) {
            ...
            val insertLambda =
                    lambdaInfo != null &&
                    completionChar != '(' &&
                    !(isReplaceCompletion && chars.isCharAt(offset, '(')) &&
                    !isSmartEnterCompletion

            val openingBracket = if (insertLambda) '{' else '('
            val closingBracket = if (insertLambda) '}' else ')'

            var insertTypeArguments =
                inputTypeArguments && (isNormalCompletion || isReplaceCompletion)
            ...
            if (openingBracketOffset == null) {
                if (insertLambda) {
                    ...
                    if (isInsertSpacesInOneLineFunctionEnabled(offsetElement.containingFile)) {
                        document.insertString(offset, " {  }")
                        inBracketsShift = 1
                    } else {
                        document.insertString(offset, " {}")
